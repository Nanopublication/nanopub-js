...
describe('Nanopub class', () => {
  ...
  it('publish throws on server error', async () => {
    ...
    await expect(np.publish('https://mock.registry/np/')).rejects.toThrow(
      /Nanopub publish failed/,
    );
  ...
  it('auto-generates provenance when none is provided', () => {
    ...
    expect(np.provenance.length).toBe(1);
    expect(np.provenance[0].predicate.value).toContain('generatedAtTime');
  });
